...
export default function RevenuePage() {
  const { t } = useI18n();
  const { bills } = useApartmentStore();

  const { rentTotal, electricityTotal, waterTotal, chartData } = useMemo(() => {
    const paidBills = bills.filter(b => b.status === 'paid');
    const rent = paidBills.filter(b => b.type === 'rent').reduce((s, b) => s + b.amount, 0);
    const elec = paidBills.filter(b => b.type === 'electricity').reduce((s, b) => s + b.amount, 0);
    const water = paidBills.filter(b => b.type === 'water').reduce((s, b) => s + b.amount, 0);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Button
          ...
          onClick={() => generateRevenueReportPDF(rentTotal, electricityTotal, waterTotal)}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">{total.toLocaleString()} Br</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t('rent')}</p>
            <p className="text-2xl font-bold text-primary">{rentTotal.toLocaleString()} Br</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t('electricity')}</p>
            <p className="text-2xl font-bold text-warning">{electricityTotal.toLocaleString()} Br</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground">{t('water')}</p>
            <p className="text-2xl font-bold text-info">{waterTotal.toLocaleString()} Br</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">{t('revenue')} — {t('overview')}</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardContent className="p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-destructive">{pendingTotal.toLocaleString()} Br</p>
          </div>
        </CardContent>
      </Card>
    </div>
